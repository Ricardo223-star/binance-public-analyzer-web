(function () {
  ...
  function renderCandlestickChart(analysis) {
    ...
    const step = visibleCandles.length ? width / visibleCandles.length : 0;
    ...
    const horizontalGrid = [0, 1, 2, 3]
      .map((gridIndex) => {
        const y = (priceHeight / 3) * gridIndex;
    ...
    return `
      <svg viewBox="0 0 ${width} ${totalHeight}" aria-label="Gráfico de velas">
        ${horizontalGrid}
        <rect x="0" y="${volumeTop}" width="${width}" height="${volumeHeight}" fill="none" stroke="#334155" stroke-opacity="0.5" />
        ${candleShapes}
